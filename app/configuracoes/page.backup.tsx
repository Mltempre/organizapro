...
const configInicial: Config = {
  ...
  logo_url: '',
  link_google: '',
  msg_lembrete: 'Ola {nome}! Lembrando sua consulta amanha dia {data} as {horario}.',
  msg_confirmacao: 'Ola {nome}! Consulta confirmada para {data} as {horario}.',
  msg_avaliacao: 'Ola {nome}! Deixe sua avaliacao: {link}',
...
export default function ConfiguracoesPage() {
  ...
  async function salvar() {
    ...
    const { data: { user } } = await supabase.auth.getUser();
    if (!user) { setErro('Nao autenticado.'); setSalvando(false); return; }
  ...
  async function testarWhatsapp() {
    ...
    const { data: { user } } = await supabase.auth.getUser();
    if (!user) { setTesteMsg('erro:Nao autenticado.'); setTestando(false); return; }
    ...
    if (data.sucesso) setTesteMsg('sucesso:Mensagem enviada com sucesso!');
    else setTesteMsg('erro:' + (data.error || JSON.stringify(data)));
    setTestando(false);
  }
