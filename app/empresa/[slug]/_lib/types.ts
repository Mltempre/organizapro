...
// preco_centavos/disponivel: colunas novas de supabase/migrations/20260920000001_pedidos_ecommerce_ia_v1.sql
// (PREPARADA, NÃO EXECUTADA) — mesmo padrão já usado por DBFaq acima para
// uma coluna que ainda não existe: tipo pronto, select("*") já em uso não
// quebra (undefined = "sem preço", tratado igual a null).
export type DBServico    = { id: string; icone: string; imagem_url: string | null; nome: string; descricao: string | null; ordem: number; preco_centavos?: number | null; disponivel?: boolean };
