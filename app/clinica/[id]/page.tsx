...
const SERVICOS_STATIC = [
  { icon:"tooth",      titulo:"Clareamento Dental",   desc:"Resultados visíveis em até 2 sessões com tecnologia LED de última geração." },
  { icon:"smile",      titulo:"Ortodontia",           desc:"Aparelhos metálicos, estéticos e alinhadores invisíveis para o sorriso ideal." },
  { icon:"gem",        titulo:"Facetas de Porcelana", desc:"Transformação completa com facetas naturais e ultra-resistentes." },
  { icon:"microscope", titulo:"Implantes Dentários",  desc:"Reposição permanente com materiais de titânio certificados." },
  { icon:"shield",     titulo:"Próteses Dentárias",   desc:"Próteses fixas e removíveis com total conforto e estética superior." },
  { icon:"sparkle",    titulo:"Limpeza Profissional", desc:"Profilaxia completa para eliminar tártaro e prevenir doenças periodontais." },
];
...
const SERVICO_SVG: Record<string, React.ReactElement> = {
  tooth: <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8"><path d="M12 2C8 2 5 5 5 8c0 2 .5 3.5 1 5l1 7c.2 1 1 1 1.5 0L9 17c.3-1 .7-1 1-1h4c.3 0 .7 0 1 1l.5 3c.5 1 1.3 1 1.5 0l1-7c.5-1.5 1-3 1-5 0-3-3-6-7-6z"/></svg>,
  ...
  microscope: <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8"><path d="M6 18h12M6 22h12M14 9a2 2 0 00-2-2H8V3h4a2 2 0 012 2v4z"/><path d="M14 9l2 9H8L10 9"/></svg>,
...
const GALERIA_STATIC = [
  { titulo:"Recepcao",    desc:"Ambiente acolhedor e climatizado para seu conforto." },
  { titulo:"Consultorio", desc:"Equipado com tecnologia de ponta para seu tratamento." },
  { titulo:"Equipamentos",desc:"Aparelhos digitais de última geração certificados." },
  { titulo:"Equipe",      desc:"Profissionais especializados e com grande experiência." },
];
const GALERIA_GRAD = [
...
const ANTES_STATIC = [
  { titulo:"Clareamento Dental",  antes:"Dentes amarelados com manchas",    depois:"Sorriso até 8 tons mais branco", cor:"#f59e0b" },
  { titulo:"Facetas de Porcelana",antes:"Dentes irregulares e desgastados",  depois:"Sorriso harmonioso e natural",   cor:"#8b5cf6" },
  { titulo:"Ortodontia",          antes:"Dentes tortos e desalinhados",      depois:"Alinhamento perfeito em meses",  cor:"#00c896" },
];
const EQUIPE_STATIC = [
  { iniciais:"CF", nome:"Dr. Carlos Ferreira",cor:"#0d3d2e",esp:"Implantodontia & Cirurgia" },
  { iniciais:"AM", nome:"Dra. Ana Mendes",    cor:"#1a3a5c",esp:"Ortodontia & Estetica" },
  { iniciais:"RB", nome:"Dr. Rafael Braga",   cor:"#2d1b69",esp:"Endodontia & Protese" },
];
const DIFERENCIAIS = [
  "Atendimento humanizado e sem julgamentos",
  "Equipamentos digitais de última geração",
  "Ambiente climatizado e confortável",
  "Equipe com especialização comprovada",
  "Orcamento gratuito e sem compromisso",
  "Parcelamento facilitado em até 12x",
];
const DEPOIMENTOS_STATIC = [
  { nome:"Ana Carolina S.", cidade:"Sao Paulo, SP",      texto:"Fiz clareamento dental e o resultado superou todas as expectativas. Atendimento incrivel!", avatar:"AC", cor:"#00c896", nota:5 },
  { nome:"Roberto Mendes",  cidade:"Rio de Janeiro, RJ", texto:"Precisei de implante e tive muito medo. A equipe foi extremamente atenciosa e o resultado ficou perfeito!", avatar:"RM", cor:"#3b82f6", nota:5 },
  { nome:"Juliana Farias",  cidade:"Curitiba, PR",       texto:"Fiz as facetas de porcelana e minha autoestima mudou completamente. Profissionalismo em cada detalhe!", avatar:"JF", cor:"#8b5cf6", nota:5 },
  { nome:"Marcos Oliveira", cidade:"Belo Horizonte, MG", texto:"Fiz a limpeza e o clareamento. Resultado incrivel em uma unica tarde. Nota mil!", avatar:"MO", cor:"#f59e0b", nota:5 },
];
const FAQS = [
  { p:"Como faco para agendar uma consulta?", r:"E simples! Clique no botao do WhatsApp em qualquer parte do site. Nossa equipe responde rapidamente e agenda no melhor horario para voce." },
  { p:"A consulta de avaliacao e gratuita?",  r:"Sim! Realizamos avaliacao inicial gratuita e sem compromisso. Voce saira com um orcamento detalhado e personalizado." },
  { p:"Quais formas de pagamento sao aceitas?",r:"Aceitamos cartoes de credito e debito, PIX e parcelamos em ate 12x sem juros nos principais procedimentos." },
  { p:"Quanto tempo dura um clareamento dental?",r:"O clareamento de consultorio dura em media 1h a 1h30. O de moldeira caseira costuma ser feito em 2 a 3 semanas." },
  { p:"Atendimento de urgencia/emergencia?",  r:"Sim, atendemos casos de urgencia. Entre em contato pelo WhatsApp para verificarmos a disponibilidade o mais rapido possivel." },
  { p:"O tratamento com aparelho doi?",       r:"Nos primeiros dias pode haver um leve desconforto de adaptacao, que passa rapidamente. Nossa equipe te orienta em cada etapa." },
];
...
function CTABand({ waLink }: { waLink: string }) {
  return (
    <section style={{ background:"linear-gradient(135deg,#00c896 0%,#00a87d 100%)", padding:"48px 20px", textAlign:"center" }}>
      ...
      <h2 style={{ fontSize:"clamp(22px,4vw,34px)", fontWeight:900, color:"#ffffff", margin:"0 0 28px", lineHeight:1.2 }}>Agende agora e transforme seu sorriso</h2>
...
export default function SiteClinica() {
  ...
  useEffect(() => {
    ...
    async function carregar() {
      ...
      if (!config?.clinica_id) { setClinica(null); setLoading(false); return; }
      const cid = config.clinica_id;

      // 2. Carrega clinica + todos os modulos em paralelo
      const [clinicaRes, galeriaRes, equipeRes, adRes, depRes, srvRes, estRes] = await Promise.all([
  ...
  const whatsappNumber = clinica.whatsapp?.replace(/\D/g,"");
  const waLink = whatsappNumber ? "https://wa.me/"+whatsappNumber+"?text=Ol%C3%A1!%20Gostaria%20de%20agendar%20uma%20consulta." : "#";
  const nome = clinica.nome || "Nossa Clinica";
  const esp  = clinica.especialidade || "Odontologia";
  ...
  return (
    <div style={{ fontFamily:"Inter,sans-serif", background:"#f8fafc", color:"#0f172a" }}>
      ...
      {/* ── HERO ──────────────────────────────────────────────────────────── */}
      <section style={{
        background: clinica.hero_url
          ? `linear-gradient(140deg,rgba(6,28,44,0.85) 0%,rgba(10,46,30,0.80) 50%,rgba(6,28,44,0.85) 100%), url(${clinica.hero_url}) center/cover no-repeat`
        ...
        <div style={{ maxWidth:1080,margin:"0 auto",padding:"110px 20px 80px",position:"relative",zIndex:1,display:"flex",flexDirection:"column",alignItems:"center",textAlign:"center" }}>
          {clinica.logo_url && (
            <div style={{ marginBottom:24 }}>
              <img src={clinica.logo_url} alt={nome} style={{ height:64,maxWidth:200,objectFit:"contain",filter:"drop-shadow(0 2px 10px rgba(0,0,0,0.50))" }}/>
            </div>
          )}
          <div style={{ display:"inline-flex",alignItems:"center",gap:8,background:"rgba(0,200,150,0.12)",border:"1px solid rgba(0,200,150,0.30)",borderRadius:50,padding:"6px 16px",marginBottom:28 }}>
            ...
            <span style={{ fontSize:12,fontWeight:700,color:"#00c896",letterSpacing:"0.08em" }}>{esp} &middot; {clinica.cidade}, {clinica.estado}</span>
          </div>
          ...
          {clinica.nota_google && (
            <div style={{ display:"flex",alignItems:"center",justifyContent:"center",gap:8,margin:"-4px 0 20px" }}>
              ...
              <span style={{ color:"#f59e0b",fontWeight:800,fontSize:17 }}>{clinica.nota_google}</span>
              {clinica.num_avaliacoes && <span style={{ color:"rgba(255,255,255,0.45)",fontSize:13 }}>({clinica.num_avaliacoes} avaliações)</span>}
            </div>
          )}
          <p style={{ fontSize:"clamp(16px,2.5vw,20px)",color:"rgba(255,255,255,0.60)",margin:"0 0 44px",maxWidth:520,lineHeight:1.65,textAlign:"center" }}>
            Agende sua avaliacao gratuita hoje e descubra o tratamento ideal para o seu sorriso.
          </p>
          <div className="hero-ctas" style={{ display:"flex",flexWrap:"wrap",gap:12,alignItems:"center",marginBottom:52 }}>
            ...
            {clinica.telefone && (
              <a href={"tel:"+clinica.telefone} style={{ display:"inline-flex",alignItems:"center",gap:8,padding:"16px 24px",borderRadius:50,border:"1.5px solid rgba(255,255,255,0.20)",color:"rgba(255,255,255,0.80)",textDecoration:"none",fontSize:15,fontWeight:500 }}>
                <IcPhone/> {clinica.telefone}
              </a>
            )}
          </div>
          <div className="hero-stats" style={{ display:"flex",gap:40,flexWrap:"wrap" }}>
            {[["98%","Satisfacao"],["200+","Pacientes"],["10+","Anos de exp."],["0 R$","Avaliacao"]].map(([v,l]) => (
              <div key={l}>
      ...
      <section style={{ padding:"72px 20px",background:"#ffffff" }}>
        <div style={{ maxWidth:1080,margin:"0 auto" }}>
          <div style={{ textAlign:"center",marginBottom:52 }}>
            ...
            <h2 style={{ fontSize:"clamp(26px,4vw,40px)",fontWeight:800,color:"#0f172a",margin:0,lineHeight:1.2 }}>Tratamentos completos para voce</h2>
            <p style={{ fontSize:15,color:"#64748b",margin:"14px auto 0",maxWidth:480,lineHeight:1.7 }}>Tecnologia avancada e equipe especializada para cada necessidade.</p>
          </div>
      ...
      <section style={{ padding:"72px 20px",background:"#f8fafc" }}>
        <div style={{ maxWidth:1080,margin:"0 auto" }}>
          <div style={{ textAlign:"center",marginBottom:48 }}>
            ...
            <h2 style={{ fontSize:"clamp(26px,4vw,40px)",fontWeight:800,color:"#0f172a",margin:0 }}>Conheca Nossa Clinica</h2>
            <p style={{ fontSize:15,color:"#64748b",margin:"14px auto 0",maxWidth:460,lineHeight:1.7 }}>Ambiente moderno, equipamentos de ponta e equipe dedicada ao seu bem-estar.</p>
          </div>
        ...
        <section style={{ padding:"72px 20px",background:"#ffffff" }}>
          <div style={{ maxWidth:1080,margin:"0 auto" }}>
            <div style={{ textAlign:"center",marginBottom:48 }}>
              ...
              <h2 style={{ fontSize:"clamp(26px,4vw,40px)",fontWeight:800,color:"#0f172a",margin:0 }}>Estrutura da Clinica</h2>
            </div>
            <div className="three-col" style={{ display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:18 }}>
              {estrutura.map(e => (
      ...
      <section style={{ padding:"72px 20px",background:showEstrutura?"#f8fafc":"#ffffff" }}>
        <div style={{ maxWidth:1080,margin:"0 auto" }}>
          <div style={{ textAlign:"center",marginBottom:48 }}>
            ...
            <p style={{ fontSize:15,color:"#64748b",margin:"14px auto 0",maxWidth:460,lineHeight:1.7 }}>Veja as transformacoes que realizamos. Cada sorriso conta uma historia.</p>
          </div>
      ...
      <section style={{ padding:"72px 20px",background:"linear-gradient(135deg,#061c2c 0%,#0a2e1e 100%)" }}>
        <div className="two-col" style={{ maxWidth:1080,margin:"0 auto",display:"grid",gridTemplateColumns:"1fr 1fr",gap:60,alignItems:"center" }}>
          <div>
            ...
            <h2 style={{ fontSize:"clamp(26px,4vw,40px)",fontWeight:800,color:"#ffffff",margin:"0 0 16px",lineHeight:1.2 }}>Cuidado que vai alem do tratamento</h2>
            <p style={{ fontSize:15,color:"rgba(255,255,255,0.55)",lineHeight:1.75,margin:"0 0 32px" }}>Acreditamos que um sorriso saudavel transforma vidas. Por isso, combinamos tecnologia de ponta com atendimento verdadeiramente humano.</p>
          ...
          <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:14 }}>
            {[["98%","Satisfacao"],["200+","Pacientes"],["10+","Anos exp."],["Gratis","Avaliacao"]].map(([v,l], i) => (
      ...
      <section style={{ padding:"72px 20px",background:"#f8fafc" }}>
        <div style={{ maxWidth:1080,margin:"0 auto" }}>
          <div style={{ textAlign:"center",marginBottom:48 }}>
            ...
            <p style={{ fontSize:15,color:"#64748b",margin:"14px auto 0",maxWidth:440,lineHeight:1.7 }}>Profissionais dedicados a oferecer o melhor cuidado para o seu sorriso.</p>
          </div>
          <div className="three-col" style={{ display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:20 }}>
            {showEquipe
              ? equipe.map((m, i) => (
      ...
      <section style={{ padding:"72px 20px",background:"#ffffff" }}>
        <div style={{ maxWidth:1080,margin:"0 auto" }}>
          <div style={{ textAlign:"center",marginBottom:48 }}>
            ...
            <h2 style={{ fontSize:"clamp(26px,4vw,40px)",fontWeight:800,color:"#0f172a",margin:0 }}>O que nossos pacientes dizem</h2>
          </div>
          <div className="four-col" style={{ display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:18 }}>
          ...
          <div className="two-col" style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:28,alignItems:"start" }}>
            <div style={{ display:"flex",flexDirection:"column",gap:12 }}>
              {clinica.endereco && (
                <div style={{ display:"flex",alignItems:"flex-start",gap:14,background:"#ffffff",borderRadius:14,padding:"18px 20px",border:"1.5px solid #e2e8f0" }}>
                  ...
                  <div>
                    ...
                    <div style={{ fontSize:15,color:"#0f172a",fontWeight:600 }}>{clinica.endereco}</div>
                    {clinica.cidade && <div style={{ fontSize:13,color:"#64748b",marginTop:2 }}>{clinica.cidade}, {clinica.estado}</div>}
                  </div>
                </div>
              )}
              {clinica.telefone && (
                <a href={"tel:"+clinica.telefone} style={{ display:"flex",alignItems:"center",gap:14,background:"#ffffff",borderRadius:14,padding:"18px 20px",border:"1.5px solid #e2e8f0",textDecoration:"none" }}>
                  ...
                  <div>
                    ...
                    <div style={{ fontSize:15,color:"#0f172a",fontWeight:600 }}>{clinica.telefone}</div>
                  </div>
                </a>
              )}
              {clinica.email && (
                <a href={"mailto:"+clinica.email} style={{ display:"flex",alignItems:"center",gap:14,background:"#ffffff",borderRadius:14,padding:"18px 20px",border:"1.5px solid #e2e8f0",textDecoration:"none" }}>
                  ...
                  <div>
                    ...
                    <div style={{ fontSize:15,color:"#0f172a",fontWeight:600 }}>{clinica.email}</div>
                  </div>
                </a>
              )}
              {whatsappNumber && (
              ...
              {clinica.google_maps_url && (
                <a href={clinica.google_maps_url} target="_blank" rel="noreferrer" style={{ display:"flex",alignItems:"center",justifyContent:"center",gap:10,padding:"14px 20px",borderRadius:12,background:"#4285F4",color:"#ffffff",textDecoration:"none",fontSize:14,fontWeight:600 }}>
                  Ver no Google Maps
                </a>
              )}
            </div>
            <div style={{ background:"#e2e8f0",borderRadius:20,minHeight:280,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:14,border:"1.5px solid #e2e8f0" }}>
              ...
              <p style={{ fontSize:14,color:"#64748b",textAlign:"center",margin:0,padding:"0 28px",lineHeight:1.6 }}>{clinica.endereco || "Localizacao nao cadastrada"}</p>
              {clinica.google_maps_url && (
                <a href={clinica.google_maps_url} target="_blank" rel="noreferrer" style={{ padding:"10px 22px",borderRadius:10,background:"#00c896",color:"#ffffff",textDecoration:"none",fontSize:13,fontWeight:700 }}>
                  Ver no mapa
                </a>
              )}
            </div>
      ...
      {whatsappNumber && (
        <section style={{ background:"linear-gradient(135deg,#061c2c 0%,#0a2e1e 100%)",padding:"88px 20px",textAlign:"center",position:"relative",overflow:"hidden" }}>
          ...
          <div style={{ maxWidth:680,margin:"0 auto",position:"relative",zIndex:1 }}>
            <p style={{ fontSize:12,fontWeight:700,color:"#00c896",letterSpacing:"0.12em",textTransform:"uppercase",margin:"0 0 16px" }}>Pronto para transformar seu sorriso?</p>
            ...
            <p style={{ fontSize:16,color:"rgba(255,255,255,0.50)",margin:"0 0 40px",lineHeight:1.7 }}>Nossa equipe esta pronta para receber voce com atencao e carinho. Respondemos em minutos.</p>
            <a href={waLink} target="_blank" rel="noreferrer" style={{ display:"inline-flex",alignItems:"center",gap:12,padding:"18px 42px",borderRadius:50,background:"linear-gradient(135deg,#25D366,#128C7E)",color:"#ffffff",textDecoration:"none",fontWeight:800,fontSize:17,boxShadow:"0 12px 40px rgba(37,211,102,0.40)" }}>
              <IcWa/> Falar com a Clinica no WhatsApp
            </a>
        ...
        <div style={{ maxWidth:1080,margin:"0 auto" }}>
          <div className="footer-cols" style={{ display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:20,marginBottom:24 }}>
            <div>
              ...
              <div style={{ fontSize:13,color:"rgba(255,255,255,0.35)" }}>{esp}</div>
            </div>
            <div style={{ display:"flex",flexDirection:"column",gap:8 }}>
              {clinica.telefone && <a href={"tel:"+clinica.telefone} style={{ display:"flex",alignItems:"center",gap:8,color:"rgba(255,255,255,0.50)",textDecoration:"none",fontSize:13 }}><IcPhone/> {clinica.telefone}</a>}
              ...
              {clinica.email && <a href={"mailto:"+clinica.email} style={{ display:"flex",alignItems:"center",gap:8,color:"rgba(255,255,255,0.50)",textDecoration:"none",fontSize:13 }}><IcMail/> {clinica.email}</a>}
            </div>
            {clinica.endereco && (
              <div style={{ display:"flex",alignItems:"flex-start",gap:8,color:"rgba(255,255,255,0.40)",fontSize:13,maxWidth:220 }}>
                ...
                <span>{clinica.endereco}{clinica.cidade ? " - "+clinica.cidade+", "+clinica.estado : ""}</span>
              </div>
            )}
          </div>
          <div style={{ borderTop:"1px solid rgba(255,255,255,0.06)",paddingTop:18,display:"flex",justifyContent:"space-between",flexWrap:"wrap",gap:10,alignItems:"center" }}>
            ...
            <span style={{ fontSize:12,color:"rgba(0,200,150,0.50)",fontWeight:600 }}>Site criado com ClinicaFlow</span>
          </div>
        </div>
      </footer>
    </div>
