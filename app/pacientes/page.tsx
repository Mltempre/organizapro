...
const camposForm = [
  ...
  { key: 'proxima_consulta', label: 'Proximo compromisso',       type: 'date',  placeholder: ''                                          },
  { key: 'endereco',         label: 'Endereco',                  type: 'text',  placeholder: 'Ex: Rua das Flores, 123, São Paulo - SP'   },
...
const formInicial: FormData = {
  ...
  proxima_consulta: '', status: 'ativo', endereco: '', site: '',
};
...
const contactBtn: React.CSSProperties = {
  ...
  padding: '4px 10px',
  borderRadius: 6,
  border: '1px solid rgba(31,78,95,0.4)',
...
export default function PacientesPage() {
  ...
  const carregar = useCallback(async () => {
    try {
      ...
      setErro(message || 'Erro ao carregar clientes');
    } finally {
      setCarregando(false);
    }
  }, [router]);
  ...
  // Cria ou atualiza o agendamento correspondente à proxima_consulta do cliente.
  ...
  async function salvar() {
    ...
    if (!editando && form.telefone.trim()) {
      const telefoneLimpo = normalizar(form.telefone);
      const { data: existente } = await supabase
        ...
        .maybeSingle();
      if (existente) {
        setErro(`Já existe um cliente com este telefone: ${existente.nome}`);
        return;
      }
    }
    ...
    try {
      ...
      const whatsappSalvo = form.whatsapp ? normalizar(form.whatsapp) : null;
      const payload = {
        ...form,
  ...
  async function excluir(id: string) {
    ...
    let aviso = '';
    const telefoneLimpo = normalizar(paciente.telefone || '');
    if (telefoneLimpo) {
      ...
      const { count } = await supabase
        ...
        .not('status', 'in', '("cancelado","concluido","faltou")');
      if (count && count > 0)
        aviso = `\n\n⚠️ Atenção: ${count} compromisso(s) futuro(s) vinculado(s) a este cliente.`;
    }

    if (!confirm(`Excluir o cliente "${paciente.nome}"?${aviso}`)) return;
    setExcluindo(id);
    try {
  ...
  return (
    ...
    >
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(180px,1fr))', gap:16, marginBottom:28 }}>
        {[
          { label:'Total de Clientes',  valor:pacientes.length,                                       cor:'#1F4E5F' },
          { label:'Clientes Ativos',    valor:pacientes.filter(p => p.status==='ativo').length,        cor:'#16a34a' },
          { label:'Compromissos Hoje',  valor:pacientes.filter(p => p.proxima_consulta===hoje).length, cor:'#0ea5e9' },
        ].map(c => (
      ...
      <div style={{ marginBottom:20 }}>
        <input
          ...
          placeholder="Buscar por nome, telefone ou e-mail do cliente..."
          value={busca}
      ...
      <div style={{ background:'#1e2130', borderRadius:12, border:'1px solid #2d3148', overflow:'hidden' }}>
        {carregando ? (
          ...
          <div style={{ textAlign:'center', padding:'60px 20px', color:'#475569' }}>
            ...
            <div style={{ fontSize:15, fontWeight:600 }}>Nenhum cliente encontrado</div>
            ...
            <table style={{ width:'100%', minWidth:580, borderCollapse:'collapse' }}>
              <thead>
                <tr style={{ background:'#161827' }}>
                  {['Cliente','Contato','Proximo Compromisso','Status','Acoes'].map(h => (
              ...
              <tbody>
                {filtrados.map(p => {
                  ...
                  const telWa = normalizar(p.whatsapp || p.telefone || '');
                  const temContato = p.telefone || p.whatsapp || p.email || p.endereco || p.site;
                  const bordaInfo = temContato ? 'none' : '1px solid #1a1d2e';
                  return (
                    <React.Fragment key={p.id}>
                      <tr>
                        <td style={{ padding:'14px 16px', fontSize:13, borderBottom:bordaInfo }}>
                        ...
                        <td style={{ padding:'14px 16px', fontSize:13, borderBottom:bordaInfo, color:'#cbd5e1' }}>{p.telefone || '-'}</td>
                        <td style={{ padding:'14px 16px', fontSize:13, borderBottom:bordaInfo, color: p.proxima_consulta === hoje ? '#fbbf24' : '#cbd5e1', fontWeight: p.proxima_consulta === hoje ? 600 : 400 }}>
                      ...
                      {temContato && (
                        <tr>
                          <td colSpan={5} style={{ padding:'0 16px 12px', borderBottom:'1px solid #1a1d2e' }}>
                            <div style={{ display:'flex', gap:6, flexWrap:'wrap', alignItems:'center' }}>
                              <span style={{ fontSize:10, color:'#475569', fontWeight:600, textTransform:'uppercase', letterSpacing:'0.05em', marginRight:2 }}>Contatos:</span>
                              {p.telefone && (
                                <a href={`tel:${normalizar(p.telefone)}`} style={contactBtn}>☎️ Ligar</a>
                              )}
                              {telWa && (
                                <a href={`https://wa.me/55${telWa}?text=${encodeURIComponent('Olá, tudo bem?')}`} target="_blank" rel="noreferrer" style={contactBtn}>💬 WhatsApp</a>
                              )}
                              {p.email && (
                                <a href={`mailto:${p.email}`} style={contactBtn}>📧 E-mail</a>
                              )}
                              {p.endereco && (
                                <a href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(p.endereco)}`} target="_blank" rel="noreferrer" style={contactBtn}>📍 Google Maps</a>
                              )}
                              {p.site && (
                                <a href={p.site.startsWith('http') ? p.site : `https://${p.site}`} target="_blank" rel="noreferrer" style={contactBtn}>🌐 Site</a>
                              )}
                            </div>
                          </td>
                        </tr>
      ...
      {modal && (
        <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.7)', display:'flex', alignItems:'center', justifyContent:'center', zIndex:1000 }} onClick={e => { if (e.target === e.currentTarget) setModal(false); }}>
          <div style={{ background:'#1e2130', borderRadius:16, padding:32, width:'100%', maxWidth:480, maxHeight:'90vh', overflowY:'auto', border:'1px solid #2d3148' }}>
            <h2 style={{ fontSize:18, fontWeight:700, color:'#f1f5f9', marginBottom:24, marginTop:0 }}>{editando ? 'Editar cliente' : 'Novo cliente'}</h2>
            ...
            <div style={{ display:'flex', gap:10, marginTop:24 }}>
              ...
              <button onClick={salvar} disabled={salvando} style={{ flex:2, padding:'10px', borderRadius:8, border:'none', background:'linear-gradient(135deg,#1F4E5F,#0d3547)', color:'#fff', fontSize:13, fontWeight:600, cursor:'pointer', opacity:salvando ? 0.7 : 1 }}>{salvando ? 'Salvando...' : editando ? 'Salvar alteracoes' : 'Adicionar cliente'}</button>
            </div>
          </div>
        </div>
      )}
