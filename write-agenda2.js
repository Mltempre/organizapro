...
function prevMes(mes: number, ano: number, setMes: (n:number)=>void, setAno: (n:number)=>void) {
  if(mes===0){ setMes(11); setAno(ano-1); } else { setMes(mes-1); }
}
function nextMes(mes: number, ano: number, setMes: (n:number)=>void, setAno: (n:number)=>void) {
