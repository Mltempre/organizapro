      ...
      <div style={{background:"#fff",borderRadius:20,padding:"40px 36px",width:400,boxShadow:"0 25px 50px rgba(0,0,0,0.15)"}}>
        ...
        <div style={{marginBottom:16}}>
          ...
          <input
            ...
            style={{width:"100%",padding:"12px 14px",borderRadius:10,border:"1.5px solid #e2e8f0",fontSize:14,outline:"none",boxSizing:"border-box",transition:"border 0.2s"}}
          />
        </div>
        ...
        <button
          onClick={entrar}
          disabled={loading}
          style={{width:"100%",padding:"13px",borderRadius:10,border:"none",background:"linear-gradient(135deg,#667eea,#764ba2)",color:"#fff",fontSize:15,fontWeight:700,cursor:loading?"not-allowed":"pointer",marginTop:8,opacity:loading?0.8:1}}
        >{loading?"Entrando...":"Entrar"}</button>

        <div style={{textAlign:"center",marginTop:20,fontSize:12,color:"#94a3b8"}}>
          ClínicaFlow © 2026 · Plataforma para clínicas
        </div>
      </div>
    </div>
  );
